(function(root, factory){
	...
	Module(function (Tone) {
	    ...
	    Tone.Sampler.prototype.triggerAttackCentered = function (note, center, time, velocity) {
			// console.log('note', note);
			var midi = Tone.Frequency(note).toMidi();
			const midiCentered = Tone.Frequency(note).toMidi();
			// console.log('midi', midi);
			// console.log('midi frequency', Tone.Frequency.mtof(midi));
	        // find the closest note pitch
	        var difference = this._findClosest(midi);
	        if (difference !== null) {
	            var closestNote = midi - difference;
				var buffer = this._buffers.get(closestNote);
				// console.log(difference, closestNote, Tone.intervalToFrequencyRatio(difference), note / Tone.Frequency.mtof(midi));
	            // play that note
	            var source = new Tone.BufferSource({
	                'buffer': buffer,
	                'playbackRate': note / center,
	                'fadeIn': this.attack,
	                'fadeOut': this.release,
	                'curve': 'exponential'
	            }).connect(this.output);
